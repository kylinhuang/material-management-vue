		...
		<el-card class="box-card">
			...
			<el-row :gutter="6">
				...
				<el-col :span="8">
					...
					<!--</el-button>-->
					<el-button size="small" icon="el-icon-refresh" @click="getGoodsList">刷新</el-button>
				</el-col>
			</el-row>
				...
				>
					...
					<el-table-column label="基本信息"    >
						<template slot-scope="scope"   >
							...
							<div>公司/职务: {{scope.row.nickname}}</div>
					...
					<el-table-column label="商户信息" >
						<template slot-scope="scope"   >
							<div>公司名称: {{scope.row.companyName}}</div>
							<div>备注: {{scope.row.remark}}</div>

							<div>对公账户姓名: {{scope.row.bankAccountName}}</div>
							<div>对公账户账号: {{scope.row.bankAccount}}</div>
							<div>对公账户银行: {{scope.row.bankName}}</div>

						</template>
					</el-table-column>

					<el-table-column prop="businessLicenseImgUrl" label="营业执照" align="center" width="150px" padding="0px">

						<template slot-scope="scope">
							<el-popover placement="right" trigger="hover">

								<img :src="scope.row.businessLicenseImgUrl"
								     style="height: 200px;width: 200px"/>

								<img slot="reference" :src="scope.row.businessLicenseImgUrl"
								     :alt="scope.row.businessLicenseImgUrl" style="height: 80px;width: 80px;cursor: pointer">
							</el-popover>
						</template>
					</el-table-column>
					...
					<el-table-column prop="status" label="状态" width="100" >
						<template slot-scope="scope">
							<el-tag size="mini" effect="plain" type="danger" v-if="scope.row.status==0">创建</el-tag>
							<el-tag size="mini" effect="plain" type="primary" v-if="scope.row.status==1">审核中</el-tag>
							<el-tag size="mini" effect="plain" type="warning" v-if="scope.row.status==2">审核通过</el-tag>
						</template>
					</el-table-column>



					<el-table-column prop="inBlackList" label="是否加入黑名单" >
						<template slot-scope="scope">
							<el-switch v-model="scope.row.inBlackList" @change="changUserStatus(scope.row)"></el-switch>
						</template>
					</el-table-column>
			...
			<!--&lt;!&ndash; 修改 弹出框 &ndash;&gt;-->
			<!--<el-dialog title="修改商品信息" :visible.sync="editDialogVisible" width="50%" @close="closeEditDialog">-->
				<!--<span>-->
					<!--<el-form-->
							<!--size="mini"-->
							<!--:model="editRuleForm"-->
                        ...
                        <!--</el-col>-->
							<!--<el-col :span="12">-->
	                            <!--<div class="grid-content bg-purple-light">-->
	                                <!--<el-form-item label="请选择商品" prop="model">-->
	                                    <!--<el-select v-model="editRuleForm.productId" placeholder="请选择商品">-->
	                                        <!--<el-option-->
						...
						<!--<el-form-item label="物资图片" prop="name">-->
							<!--&lt;!&ndash; 图片上传部分 &ndash;&gt;-->
							<!--<el-upload-->
									<!--v-model="editRuleForm.merchant_image_url"-->
									<!--:action="uploadApi"-->
									<!--:class="{ disabled: uploadDisabled }"-->
									<!--list-type="picture-card"-->
									<!--:limit="limitCount"-->
				...
				<!--<span slot="footer" class="dialog-footer">-->
                    ...
                    <!--<el-button type="primary" @click="editGoods" :disabled="btnDisabled" :loading="btnLoading">确 定</el-button>-->
                <!--</span>-->
			<!--</el-dialog>-->
    ...
    export default {
        ...
        methods: {
            ...
            cellStyle(row,column,rowIndex,columnIndex){//根据报警级别显示颜色
                ...
                if (row.row.price > row.row.proposedPrice) {

                    console.log("tableRowClassName price " + row.row.price   );

                    console.log("tableRowClassName proposedPrice " + row.row.proposedPrice   );
            ...
            async changUserStatus(row) {
                const { data: res } = await this.$http.put(
                    "system/user/updateBlackListStatus/" + row.userId + "/" + row.inBlackList
                );
                if(!res.success){
                    this.$message.error("更新黑名单失败:" + res.data.errorMsg);
                    row.inBlackList = !row.inBlackList;
                } else {
                    const message = row.inBlackList ? '成功加入黑名单' : '成功移除黑名单';
                    this.$notify.success({
                        title: '操作成功',
                        message: message,
                    });
            ...
            async getWarehousingSupplies() {
                ...
                // this.queryMap.status = -1;
                const {data: res} = await this.$http.get("merchantInfo/findList", {
